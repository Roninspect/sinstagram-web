...
import { account, appwriteConfigs, avatars, db, storage } from "../config";
...
export async function saveUserToDB(user: {
  ...
  try {
    const newUser = await db.createDocument(
      appwriteConfigs.databaseId,
      appwriteConfigs.userCollectionId,
      ID.unique(),
      user
    );
...
export async function createPost(post: INewPost) {
  try {
    ...
    const newPost = await db.createDocument(
      appwriteConfigs.databaseId,
      appwriteConfigs.postCollectionId,
      ID.unique(),
      {
        creator: post.userId,
...
export async function deleteFile(fileId: string) {
  try {
    await storage.deleteFile(appwriteConfigs.storageId, fileId);
...
export async function savePost(userId: string, postId: string) {
  try {
    const updatedPost = await db.createDocument(
      appwriteConfigs.databaseId,
      appwriteConfigs.savesCollectionId,
      ID.unique(),
      {
        user: userId,
        post: postId,
